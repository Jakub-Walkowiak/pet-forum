...
FollowRouter.post('/:followed_id(\\d+)', authMandatory, (req, res, next) => {
    const sql = 'INSERT INTO follow (followed_id, follower_id) VALUES ($1, $2)'

    pool.query(sql, [req.params.followed_id, req.body.id])
...
FollowRouter.delete('/:followed_id(\\d+)', authMandatory, (req, res, next) => {
