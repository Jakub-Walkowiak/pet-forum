...
AccountRouter.post('/register', async (req, res, next) => {
    try {
        ...
        const registerSql =
            'INSERT INTO user_account (email, password, account_name, display_name) VALUES ($1, $2, $3, $4)'
        ...
        pool.query(sql)
            ...
            .catch(async (err) => {
                if (err.code === '23505') {
                    ...
                    const accountNameDupe =
                        (await pool.query(accountNameDupeSql, [details.accountName])).rows[0].dupe > 0
...
AccountRouter.patch('/password', authMandatory, async (req, res, next) => {
    try {
        ...
        if (verified) {
            ...
            pool.query(changeSql, [hashedPassword, req.body.id]).then(() => res.status(204).send())
        } else {
...
AccountRouter.get('/', authOptional, async (req, res, next) => {
    try {
        const { contains, limit, offset, orderBy, orderMode, relatedTo, relationType, followsPet } =
            AccountFetchValidator.parse(req.query)
        ...
        if (relationType === RelationType.FOLLOWED || relationType === RelationType.MUTUALS) {
            ...
            const shouldFetch =
                (req.body.auth && req.body.id === relatedTo) ||
                (await pool.query(privacySql, [relatedTo])).rows[0].followed_visible
        ...
        const orderByString =
            orderBy !== AccountOrderByOption.USER_DATE_FOLLOWED && orderBy !== AccountOrderByOption.PET_DATE_FOLLOWED
                ? `${orderBy}`
                : orderBy === AccountOrderByOption.PET_DATE_FOLLOWED
                  ? 'pets.date_followed'
                  : relatedTo === undefined
                    ? AccountOrderByOption.FOLLOWERS
                    : relationType === RelationType.FOLLOWERS
                      ? 'followers.date_followed'
                      : relationType === RelationType.FOLLOWED
                        ? 'followed.date_followed'
...
AccountRouter.get('/:id(\\d+)', authOptional, async (req, res, next) => {
    try {
        ...
        if (req.body.auth === true) {
            const followedSql =
                'SELECT Count(*) AS count FROM account_follow WHERE follower_id = $1 AND followed_id = $2'
