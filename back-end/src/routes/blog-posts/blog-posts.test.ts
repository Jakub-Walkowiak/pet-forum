...
describe('/blog-posts', () => {
    ...
    describe('/ POST', () => {
        it('should fail without contents (400)', (done) => {
            agent.post('/blog-posts/').expect(400, done)
        })

        it('should require authorization (401)', (done) => {
            request(app).post('/blog-posts/').expect(401, done)
        })

        it('should fail with not owned pets (403)', () =>
            agent
                .post('/blog-posts/')
                .send({ contents, pets: [1, 6, 7] })
                .expect(403)
                .then((res) => {
        ...
        it('should respond (201)', (done) => {
            agent.post('/blog-posts/').send({ contents }).expect(201, done)
        })

        it('should respond on failed linking (404)', (done) => {
            agent
                .post('/blog-posts/')
                .send({ contents, tags: [999], pictures: [999] })
                .expect(404, done)
        })

        it('should add additional content', (done) => {
            agent.post('/blog-posts/').send({ contents, tags, pets, pictures }).expect(201, done)
        })
    })

    describe('/[id] DELETE', () => {
        it('should require authorization (401)', (done) => {
            request(app).delete('/blog-posts/17').expect(401, done)
        })

        it('should respond (204)', (done) => {
            agent.delete('/blog-posts/17').expect(204, done)
        })
    })
    ...
    describe('/[id] GET', () => {
        ...
        it("should respond if blog-post doesn't exist (404)", (done) => {
            request(app).get('/blog-posts/999').expect(404, done)
        })
    ...
    describe('/[id]/likes', () => {
        describe('/ POST', () => {
            it('should require authorization (401)', (done) => {
                request(app).post('/blog-posts/1/likes/').expect(401, done)
            })

            it('should respond (204)', (done) => {
                agent.post('/blog-posts/6/likes').expect(204, done)
            })

            it('should prevent double like (409)', (done) => {
                agent.post('/blog-posts/6/likes').expect(409, done)
            })

            it('should prevent non-existent like (404)', (done) => {
                agent.post('/blog-posts/999/likes').expect(404, done)
            })
        })

        describe('/ DELETE', () => {
            it('should require authorization (401)', (done) => {
                request(app).delete('/blog-posts/6/likes').expect(401, done)
            })

            it('should respond (204)', (done) => {
                agent.delete('/blog-posts/6/likes').expect(204, done)
            })
        })
    ...
    describe('/tags', () => {
        ...
        describe('/[id] GET', () => {
            ...
            it("should respond if tag doesn't exist (404)", (done) => {
                request(app).get('/blog-pots/tags/999').expect(404, done)
            })
        })
        ...
        describe('/ POST', () => {
            ...
            it('should require authorization (401)', (done) => {
                request(app).post('/blog-posts/tags').expect(401, done)
            })
