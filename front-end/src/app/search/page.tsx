    ...
    return (
        <>
            <form className="w-full p-2 relative border-b border-zinc-700" onSubmit={(e) => { e.preventDefault(); setCurrentSearch(getValues('search')) }}>
                ...
                <AiOutlineSearch className='absolute inset-y-0 my-auto right-4 text-gray-400 text-3xl cursor-pointer hover:text-white duration-200' onClick={() => setCurrentSearch(getValues('search'))}/>
            </form>
            <TabContainer tabs={[
                { title: 'Users', element: <AccountFeed options={{ contains: currentSearch }}/> },
                { title: 'Pets', element: <PetFeed options={{ nameQuery: currentSearch }}/> },
                { title: 'Blog posts', element: <BlogFeed options={{ contains: currentSearch }}/> },
            ]}/>
        </>
    )
}