...
export default function LoginForm({ switchForm }: LoginFormProps) {
  ...
  return (
    <form
      ...
      className='flex flex-col gap-4 px-8 py-5 fixed inset-y-0 inset-x-0 m-auto z-50 h-fit w-full max-w-xl bg-gray-900 rounded-lg items-stretch'
    >
      <CloseModalButton />

      <Input
        placeholder='E-mail or account name'
        register={register}
        name='loginKey'
        error={errors.loginKey !== undefined}
      />
      <Input
      ...
      <ErrorContainer>
        {errors.loginKey && (
          <p>{errors.loginKey.message ? errors.loginKey.message : 'Valid account name or e-mail required'}</p>
        )}
      ...
      <div className='text-center scale-90 font-light'>
        Don&apos;t have an account? <Button dark text='Register' onClickHandler={() => switchForm(FormMode.Register)} />
      </div>
    </form>
  )
}
