...
export default function PanelPopup({ show }: PanelPopupProps) {
    ...
    return (
        <div className={`-z-10 duration-200 h-36 cursor-auto absolute px-1 py-2 flex flex-col w-full bg-gray-800/30 rounded-t-lg text-xl ${show ? '-translate-y-[6.75rem] opacity-1 pointer-events-auto' : 'translate-y-0 opacity-0 pointer-events-none'}`}
            onClick={e => e.stopPropagation()}>
            <div className='duration-200 w-full cursor-pointer hover:bg-gray-800 p-1 rounded-lg flex flex-1'>
                <a href={accountRoute?.path} className='w-full flex items-center'>
