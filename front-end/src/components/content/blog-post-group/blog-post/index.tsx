...
export default function BlogPost({ id, groupArgs, hideParentButton = false, afterReply }: BlogPostProps) {
  ...
  const likePost = async () => {
    try {
      ...
      showNotificationPopup(false, 'Failed to like')
    }
  }
  ...
  const unlikePost = async () => {
    try {
      ...
      showNotificationPopup(false, 'Failed to unlike')
    }
  }
  ...
  const handleAfterReply = (createdId: number) => {
    ...
    setHideConnection(!hideConnection)
    if (groupArgs && !groupArgs.rootMaximized)
      groupArgs.setPosts((old) => [...old.slice(0, groupArgs.index + 1), createdId])
  ...
  const handleRedirect = () => {
    if (
      postData &&
      (!groupArgs?.rootMaximized || (groupArgs?.rootMaximized && groupArgs.index !== groupArgs.length - 1))
    )
  ...
  return (
    <div className='w-full h-fit flex flex-col relative'>
      {groupArgs && groupArgs.index < groupArgs.length - 1 && !hideConnection && (
        <>
          <div
            className='absolute cursor-pointer left-[26px] top-12 w-6 h-full peer'
            onClick={() => groupArgs.setPosts((old) => old.slice(groupArgs.index + 1))}
          />
      ...
      {groupArgs && !hideParentButton && postData && postData.replyTo && groupArgs.index === 0 && (
        <>
          ...
          <div
            onClick={() => groupArgs.setPosts((old) => [postData.replyTo as number, ...old])}
            className='absolute top-0 w-full h-6 from-emerald-900/50 flex items-center justify-center text-4xl text-gray-400 hover:text-5xl hover:text-white hover:bg-gradient-to-b cursor-pointer duration-200'
          >
            <AiOutlineEllipsis />
          </div>
        </>
      ...
      <div
        className={`w-full h-fit p-4 gap-4 flex ${groupArgs?.rootMaximized && groupArgs.index === groupArgs.length - 1 && 'flex-col'} ${postData && (!groupArgs?.rootMaximized || (groupArgs?.rootMaximized && groupArgs.index !== groupArgs.length - 1)) && 'cursor-pointer hover:bg-black/10 duration-200'}`}
      ...
      >
        {postData ? (
          <BlogPostBody
            ...
            showReplyCreator={handleShowReply}
            maximized={groupArgs?.rootMaximized && groupArgs.index === groupArgs.length - 1}
          />
        ) : (
          <BlogPostError />
        )}
      ...
      {postData && replyCreator && (
        <div className='p-4 pt-0'>
          <PostCreator
            placeholder={`Replying to @${posterAccountName}`}
            replyTo={id}
            maxRows={5}
            afterSubmit={handleAfterReply}
          />
        </div>
      )}
    </div>
