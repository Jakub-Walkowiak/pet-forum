...
export default function DynamicFeed<T>({ generator, generatorOptions, mapper }: DynamicFeedOption<T>) {
  ...
  return (
    <ul className='w-full flex flex-col list-none divide-y divide-zinc-700'>
      {items.map(mapper)}
      {visualLoading ? (
        <li className='w-full flex items-center justify-center h-32 mt-1 text-5xl'>
          <AiOutlineLoading className='animate-spin' />
        </li>
      ) : (
        <li className='w-full flex items-center justify-center h-32 mt-1 text-xl text-gray-500'>
