...
export default function PetHeaderEdit({ id }: PetHeaderEditProps) {
  ...
  const handleSpeciesEdit = (e: React.MouseEvent) => {
    stopEvent(e)
    showFloatingElement(
      <SelectablePetTypeForm chosen={watchType} set={(choice) => setValue('type', choice)} />,
      e.clientX,
      e.clientY,
    )
  }
