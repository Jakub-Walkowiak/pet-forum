      ...
      <div className='w-full'>
        ...
        <div className='w-full relative flex p-2 gap-2'>
          ...
          <div className='absolute top-36 left-3'>
            <AccountFollowButton id={id} followed={data.followed} onChange={setClientFollow} />
          </div>
        </div>
        ...
        <div className='flex px-3 py-4 items-end gap-3'>
          ...
          {showStats ? (
            <AiFillInfoCircle
              className='text-xl inset-y-0 my-auto cursor-pointer hover:text-gray-500 duration-200'
              onClick={() => setShowStats(false)}
            />
          ) : (
            <AiOutlineInfoCircle
              className='text-xl text-gray-500 inset-y-0 my-auto cursor-pointer hover:text-white duration-200'
        ...
        <div
          className={`${!showStats ? 'h-0' : 'h-14 xs:h-16 sm:h-10'} text-sm xs:text-base overflow-hidden duration-200`}
        >
          <div
            className={`p-2 grid grid-cols-2 grid-rows-2 sm:grid-cols-4 sm:grid-rows-1 grid-flow-col overflow-hidden bg-black/20`}
          >
            <ValueAndText value={data.blogPostCount} text='Blog posts' />
            <ValueAndText value={data.replyCount} text='Replies' />
            <ValueAndText
              value={data.accountsFollowedCount}
              text='Accs. followed'
              onClick={data.followedVisible || id === auth ? redirectFollowedAccounts : undefined}
            />
            <ValueAndText
              value={data.petsFollowedCount}
              text='Pets followed'
              onClick={data.followedVisible || id === auth ? redirectFollowedPets : undefined}
            />
          </div>
        </div>
      </div>
