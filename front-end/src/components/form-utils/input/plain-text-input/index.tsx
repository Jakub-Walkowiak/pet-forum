...
export default function PlainTextInput<T extends FieldValues>({ placeholder, error, register, name }: PlainTextInputProps<T>) {
    const styles = error
    ...
    return register !== undefined && name !== undefined
        ? <input placeholder={placeholder} className={styles} {...register(name)}/>
        : <input placeholder={placeholder} className={styles}/>
}