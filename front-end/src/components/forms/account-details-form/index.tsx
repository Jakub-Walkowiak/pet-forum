...
import patchDetails, { PatchDetailsInputs, PatchDetailsInputsValidator } from "@/helpers/fetch-helpers/account/patch-details"
    ...
    return (
        <form className="flex flex-col gap-4 py-4 w-fit" onSubmit={handleSubmit(async data => await onSubmit(data))}>
            ...
            <label className="w-fit">
                ...
                <Input placeholder={data?.accountName ? data.accountName : 'Couldn\'t fetch current account name'} register={register} name='accountName'/>
            </label>
            ...
            <div className="w-fit"><Button text={'Save changes'} loading={loading} disabled={isError()}/></div>
        </form>
    )
}