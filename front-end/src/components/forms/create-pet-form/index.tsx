...
const PetInputsValidator = z.object({
  name: z.string().trim().min(1, { message: 'Name required' }).max(50, { message: 'Max. name length is 50 chars' }),
...
export default function CreatePetForm() {
  ...
  const handleSpeciesButtonClick = (e: React.MouseEvent) => {
    ...
    stopEvent(e)
    showFloatingElement(
      //@ts-ignore <- this is an intentional 'incorrect' assignment, but I never figured out how to do it in a typesafe way
      <SelectablePetTypeForm chosen={getValues().type} set={(choice) => setValue('type', choice)} />,
      e.clientX,
      e.clientY,
    )
  }
