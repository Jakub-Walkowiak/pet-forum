...
export default function SelectableForm<T>({
  added,
  selectable,
  loading = false,
  onScroll,
  placeholder,
}: SelectableFormProps<T>) {
  ...
  const handleUnAdd = (name: string) => {
    setUnAddTimeouts((old) =>
      old?.set(
        name,
        setTimeout(() => added?.set((old) => old.filter((item) => item !== name)), 350),
      ),
    )
  }
  ...
  return (
    <form
      className='flex flex-col w-60 border border-gray-600 rounded-lg bg-gray-800 shadow-md shadow-black/70'
      onClick={stopEvent}
      onScroll={stopEvent}
    >
      <ul className='list-none flex flex-col h-40 w-full bg-black/10 overflow-y-auto' onScroll={handleScroll}>
          ...
          .map((name) => (
            <li key={`${name}`}>
              <LabeledCheckbox
                ...
                value={name}
                postfix='new'
                uncheckHandler={handleUnAdd}
                checkHandler={handleReAdd}
              />
            </li>
          ))}
        ...
        {selectable.values
          .filter((id) => !selectable.selected.includes(id))
          .map((id) => (
            <li key={`${id}`}>{selectable.displayer(id, handleSelect, handleDeselect, false)}</li>
          ))}

        {loading ? (
          <li className='w-full flex items-center justify-center mb-2 mt-1 text-xl'>
            <AiOutlineLoading className='animate-spin' />
          </li>
        ) : (
          <li className='w-full flex items-center justify-center mb-2 mt-1 text-sm text-gray-500'>
      ...
      <div className='w-full h-10 border-t border-gray-600 p-1.5 bg-black/20 relative'>
        <Input
          ...
          register={register}
          name={'name'}
          handleKeyUp={() => selectable.onQuery(getValues('name'))}
        />
        <AiOutlineSearch
          className={`absolute inset-y-0 my-auto ${added ? 'right-9' : 'right-3'} text-gray-400 text-lg cursor-pointer hover:text-white duration-200`}
          onClick={() => selectable.onQuery(getValues('name'))}
        />
        {added && (
          <AiOutlinePlusCircle
            className='absolute inset-y-0 my-auto right-3 text-gray-400 text-lg cursor-pointer hover:text-white duration-200'
            onClick={handleSubmit(
              async (data) => added.onSubmit(data.name),
              (err) => {
                showNotificationPopup(false, err.name?.message === undefined ? 'Encountered error' : err.name.message)
              },
            )}
          />
        )}
