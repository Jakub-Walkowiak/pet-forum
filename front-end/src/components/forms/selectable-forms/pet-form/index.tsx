...
export default function SelectablePetForm({ selectable }: SelectablePetFormProps) {
    ...
    const display = (id: number, handleSelect: (id: number) => void, handleDeselect: (id: number) => void, checked: boolean) => {
        return <SelectablePet petId={id} checkHandler={handleSelect} uncheckHandler={handleDeselect} createChecked={checked}/>
    }

    return !auth 
        ? <div className="flex w-60 h-40 border border-gray-600 rounded-lg bg-gray-800 justify-center align-center">Encountered error during authentication</div> 
        : <SelectableForm
            onScroll={fetchPets}
            loading={loading}
            selectable={{
