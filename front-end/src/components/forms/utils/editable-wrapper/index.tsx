...
export default function EditableWrapper<T extends FieldValues>({ innerRenderer, baseText, form, className, inputPlaceholder }: EditableWrapper<T>) {
    const showInput = (e: MouseEvent) => {
        if (form) showFloatingElement(
            <Input 
                focused 
                register={form.register} name={form.name} 
