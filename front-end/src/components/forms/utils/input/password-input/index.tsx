...
export default function PasswordInput<T extends FieldValues>({
  ...
  const showIcon = !show ? (
    <AiOutlineEye
      className='absolute right-4 inset-y-0 m-auto text-2xl text-gray-500 hover:cursor-pointer hover:text-white duration-200'
      onClick={() => setShow(true)}
    />
  ) : (
    <AiOutlineEyeInvisible
      className='absolute right-4 inset-y-0 m-auto text-2xl text-gray-500 hover:cursor-pointer hover:text-white'
      onClick={() => setShow(false)}
    />
  )

  return register !== undefined && name !== undefined ? (
    <div autoFocus={focused} className='relative'>
      <input
