...
export default function PlainTextInput<T extends FieldValues>({ placeholder, error, register, name, className, handleKeyUp, defaultValue }: PlainTextInputProps<T>) {
    const styles = error
    ...
    return register !== undefined && name !== undefined
        ? <input placeholder={placeholder} className={`${className} ${styles}`} onKeyUp={handleKeyUp} defaultValue={defaultValue} {...register(name)}/>
        : <input placeholder={placeholder} className={`${className} ${styles}`} onKeyUp={handleKeyUp} defaultValue={defaultValue}/>
}