...
export default function PlainTextInput<T extends FieldValues>({ placeholder, error, register, name, className, handleKeyUp, defaultValue, focused = false }: PlainTextInputProps<T>) {
    const styles = error
    ...
    return register !== undefined && name !== undefined
        ? <input autoFocus={focused} placeholder={placeholder} className={`${className} ${styles}`} onKeyUp={handleKeyUp} defaultValue={defaultValue} {...register(name)}/>
        : <input autoFocus={focused} placeholder={placeholder} className={`${className} ${styles}`} onKeyUp={handleKeyUp} defaultValue={defaultValue}/>
}