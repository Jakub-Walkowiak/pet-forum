...
export const PostContentsValidator = z.string().trim()
    .min(1, { message: 'Post must contain at least one character' })
    .max(300, { message: 'Post can\'t contain more than 300 characters' })

export default async function postBlogPost(contents: string, images: UploaderImages, replyTo?: number, pets?: Array<number>, tags?: { added?: Array<string>, selected?: Array<number> }): Promise<Response | PostUploadError> {
    ...
    const tagPromises = tags.added.map(name => fetch('http://localhost:3000/blog-posts/tags', {
        method: 'POST',
        credentials: 'include',
        mode: 'cors',
